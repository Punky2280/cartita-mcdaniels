/**
 * Aurora Interface - API Data Hook
...
import { AuroraApiError } from '@/services/api';
...
export interface UseApiDataOptions<T> {
  ...
  onError?: (error: AuroraApiError) => void;
}

export interface UseApiDataReturn<T> {
  data: T | null;
  loading: boolean;
  error: AuroraApiError | null;
...
export const useApiData = <T>(
  ...
  const [error, setError] = useState<AuroraApiError | null>(null);
  ...
  const executeWithRetry = useCallback(async (attempt = 1): Promise<void> => {
    try {
      ...
      const apiError = err instanceof AuroraApiError ? err : new AuroraApiError(
