/**
 * Cartrita Interface - API Service Layer
...
export class CartritaApiError extends Error {
  ...
  private async executeRequest<T>(
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (error instanceof CartritaApiError) {
          throw error;
        }
    ...
    throw new CartritaApiError('Unexpected error in request execution', 'UNKNOWN_ERROR');
  }
